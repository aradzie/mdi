/* Generated from file mixed-martial-arts.svg. */

import { SvgIcon, SvgIconProps } from "../svg-icon";
import * as React from "react";

const MixedMartialArts: React.FunctionComponent<SvgIconProps> = (props) => (
    <SvgIcon {...props}>
        <path d="M15 10V7H7v3h8m3-3c.28 0 .5.09.7.29.19.21.3.44.3.71v2.78c0 .19-.03.33-.06.42l-.8 3.99c-.14.53-.44.81-.94.81H6.8c-.53 0-.85-.28-.94-.81l-.8-3.99c-.03-.09-.06-.23-.06-.42V5c0-.5.21-1 .6-1.39C6 3.2 6.45 3 7 3h8c.53 0 1 .2 1.41.61.4.39.59.89.59 1.39v3c0-.27.11-.5.3-.71.2-.2.42-.29.7-.29M7 20v-3h10v3c0 .3-.09.53-.29.72-.21.19-.44.28-.71.28H8c-.27 0-.5-.09-.71-.28-.2-.19-.29-.42-.29-.72z"/>
    </SvgIcon>
);

MixedMartialArts.displayName = "MixedMartialArts";

export default MixedMartialArts;
